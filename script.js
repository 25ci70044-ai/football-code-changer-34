...
// --- GANG PROFILE & SETTINGS ---
function openProfile() {
    const userStr = localStorage.getItem('gang_user');
    if (!userStr) return;
    const user = JSON.parse(userStr);
    
    // Modern modal for the profile
    const modal = document.createElement('div');
    modal.className = 'game-modal-overlay';
    modal.innerHTML = `
        <div class="game-modal profile-modal">
            <span class="close-btn" onclick="this.closest('.game-modal-overlay').remove()">&times;</span>
            ...
            <div class="profile-body">
                <div class="stat-item">
                    <span>Rank</span>
                    <span class="stat-value">GANG ${user.role.toUpperCase()}</span>
                </div>
                <button onclick="changePasswordUI()" class="btn-secondary">🔑 Update Password</button>
                <button onclick="logout()" class="btn-danger">🚪 Leave Clubhouse</button>
            </div>
        </div>
    `;
...
async function changePasswordUI() {
    const newPass = prompt("Enter your new Gang Password:");
    if (!newPass || newPass.length < 3) {
        alert("❌ Password too short!");
        return;
    }
    
    const user = JSON.parse(localStorage.getItem('gang_user'));
    ...
    try {
        ...
        if (res.ok) {
            alert("✅ Gang Password updated! It will save permanently now.");
        } else {
...
function showTab(tabName) {
    ...
    if (tabName === 'roster') {
        ...
        document.getElementById('editor-tabs').style.display = activeFile ? 'flex' : 'none';
        ...
        if (activeFile) {
            ...
            placeholderEl.innerHTML = `
                <div style="text-align:center; opacity:0.7">
                    ...
                    <p>Welcome back, Coach! <br> Select a file on the left to start your tactics.</p>
                </div>
            `;
        }
    }
